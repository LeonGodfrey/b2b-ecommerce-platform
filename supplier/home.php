...
<body class="hold-transition skin-blue sidebar-mini sidebar-collapse">
    ...
    <div class="content-wrapper">
      ...
      <div class="content-header">
        <div class="container-fluid">
          <div class="row mb-2">
            <div class="col-sm-6">
              <h1 class="m-0"><?php echo $supplier['shopName']; ?></h1>
            </div><!-- /.col -->
            <div class="col-sm-6">
  ...
  $data = [];
  //get rejected orders.
  $stmt = $conn->prepare("SELECT COUNT(orderId) as numrows from orders where orders.userIdD= :id AND status = :status");
  $stmt->execute(['id' => $supplier['userIdD'], 'status' => 'rejected']);
  $row = $stmt->fetch();
  array_push($data, $row['numrows']);

  //get delivered orders.
  $stmt3 = $conn->prepare("SELECT COUNT(orderId) as numrows from orders where orders.userIdD= :id AND status = :status");
  $stmt3->execute(['id' => $supplier['userIdD'], 'status' => 'delivered']);
  $row3 = $stmt3->fetch();
  array_push($data, $row3['numrows']);

  //get confirmed orders.
  $stmt1 = $conn->prepare("SELECT COUNT(orderId) as numrows from orders where orders.userIdD= :id AND status = :status");
  $stmt1->execute(['id' => $supplier['userIdD'], 'status' => 'confirmed']);
  $row1 = $stmt1->fetch();
  array_push($data, $row1['numrows']);

  //get pending orders.
  $stmt2 = $conn->prepare("SELECT COUNT(orderId) as numrows from orders where orders.userIdD= :id AND status = :status");
  $stmt2->execute(['id' => $supplier['userIdD'], 'status' => 'pending']);
  $row2 = $stmt2->fetch();
